...
// --- DATA CONFIGURATION ---
const CATEGORIES = [
  { name: "Servers & Storage", desc: "High-performance servers and storage systems for critical business workloads, from single-server setups to multi-node deployments.", icon: Server },
  { name: "Business Laptops", desc: "Reliable, portable laptops configured for enterprise use — bulk order pricing available for teams and offices.", icon: Laptop },
  { name: "Desktops & Workstations", desc: "Desktop and workstation setups built for professional workloads, from standard office use to design/engineering-grade specs.", icon: Monitor },
  { name: "Networking", desc: "Switches, routers, and access points for dependable business connectivity — single-office to multi-branch deployments.", icon: Router },
  { name: "Printers & Scanners", desc: "Business-grade printing and scanning hardware, configured for office and retail counter environments.", icon: Printer },
  { name: "Security & Surveillance", desc: "CCTV, access control, and surveillance hardware to protect your premises and assets.", icon: Shield },
  { name: "Accessories & Peripherals", desc: "Keyboards, mice, docking stations, and other essentials to complete your business hardware setup.", icon: Keyboard },
];
...
const PRODUCTS = [
  { 
    ...
    price: "89,990",
    imgAlt: "Dell PowerEdge T150 server for enterprise workloads"
  },
  { 
    ...
    price: "74,990",
    imgAlt: "HP ProBook 440 G10 business laptop"
  },
  { 
    badge: null,
    ...
    price: "43,990",
    imgAlt: "Lenovo ThinkCentre Neo 50s business desktop"
  },
  { 
    ...
    price: "16,990",
    imgAlt: "Cisco Business 24 Port network switch"
  },
  { 
    badge: null,
    ...
    price: "8,990",
    imgAlt: "TP-Link EAP225 Wi-Fi 5 Access Point"
  },
];
    ...
    <main className="w-full bg-[#FAFAFA] font-sans text-gray-900">
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6 max-w-[1400px]">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-10 text-center lg:text-left">Shop by Category</h2>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {CATEGORIES.map((cat, i) => (
              <div key={i} className="group bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-xl transition-all cursor-pointer flex flex-col h-full shadow-sm">
                ...
                <h3 className="text-base font-bold text-gray-900 leading-tight mb-2">{cat.name}</h3>
                <p className="text-xs text-gray-500 leading-relaxed mb-6">{cat.desc}</p>
      ...
      <section className="py-20 bg-[#FAFAFA] border-t border-gray-100">
        <div className="container mx-auto px-6 max-w-[1400px]">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-10">
             ...
             {/* "View All Products" button temporarily commented out/disabled as per instructions until catalog page exists
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {PRODUCTS.map((prod, i) => (
              <div key={i} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-xl transition-shadow flex flex-col relative group">
                {/* Badge */}
                {prod.badge && (
                ...
                <div className="h-40 w-full bg-gray-50 rounded-xl mb-4 flex items-center justify-center group-hover:scale-105 transition-transform overflow-hidden relative">
                   <MonitorSmartphone className="w-12 h-12 text-gray-300 stroke-[1]" />
                   {/* Fallback hidden text for alt implementation on actual images */}
                   <span className="sr-only">{prod.imgAlt}</span>
                </div>
                ...
                <div className="mb-4">
                  <div className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold mb-0.5">Starting at</div>
                  <div className="text-xl font-bold text-gray-900">₹ {prod.price}</div>
                </div>
                
                <div className="flex flex-col xl:flex-row gap-2">
                  <button className="w-full bg-[#3b82f6] hover:bg-blue-600 text-white text-xs font-bold py-2.5 rounded-lg transition-colors">
                    Get Quote
                  </button>
                  <button className="w-full border border-green-500 text-green-600 hover:bg-green-50 text-xs font-bold py-2.5 rounded-lg transition-colors flex items-center justify-center gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-8">
            {FEATURES.map((feat, i) => (
              <div key={i} className="flex items-start gap-4 p-4 rounded-xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/10">
                ...
                <div>
                  <h3 className="text-sm md:text-base font-bold text-gray-100 mb-1.5">{feat.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {INDUSTRIES.map((ind, i) => (
              <Link href={ind.link} key={i} className="group relative bg-[#F8FAFC] border border-gray-100 rounded-2xl p-6 hover:shadow-lg transition-all flex flex-col items-center text-center">
                ...
                <h3 className="text-sm font-bold text-gray-900 mb-2">{ind.title}</h3>
                <p className="text-[10px] text-gray-500 leading-relaxed">{ind.desc}</p>
              </Link>
            ))}
          </div>
          ...
          <div className="mt-16 pt-10 border-t border-gray-100 flex flex-col md:flex-row items-center justify-between gap-8">
             ...
             <div className="flex flex-wrap items-center justify-center md:justify-end gap-6 md:gap-10 opacity-60 grayscale">
               {["Dell", "HP", "Lenovo", "Microsoft", "Cisco", "APC", "Logitech", "Intel", "Samsung"].map((brand) => (
                 <div key={brand} className="text-base sm:text-lg font-bold text-gray-800">{brand}</div>
               ))}
             </div>
          </div>
        </div>
