              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12 pt-8 border-t border-gray-100">
                {HERO_FEATURES.map((feat, i) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] font-bold text-gray-900 uppercase tracking-wide">
                      {feat.title}
                    </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {DEPARTMENTS.map((dept, i) => (
              <Link
                href={dept.link}
                key={i}
                className="group bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-xl hover:border-[#1881c4]/30 transition-all flex flex-col cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {INDUSTRIES.map((ind, i) => (
              <Link
                href={ind.link}
                key={i}
                className="group relative h-[320px] rounded-2xl overflow-hidden cursor-pointer shadow-sm block"
              >
                <Image
                  src={ind.image}
                  alt={ind.title}
        ...
        <div className="container mx-auto px-6 max-w-[1400px]">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 bg-white border border-gray-200 rounded-3xl p-8 flex flex-col justify-between hover:shadow-lg transition-shadow">
              <div>
                <div className="text-4xl text-[#1881c4] font-serif leading-none mb-4">
                  "
                </div>
              ...
              <div className="flex items-center justify-between border-t border-gray-100 pt-6">
                ...
                <div className="flex gap-2">
                  <button className="w-7 h-7 rounded-full bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-gray-400 transition-colors cursor-default">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                {IMPACT_METRICS.map((metric, i) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] text-gray-400 leading-snug whitespace-pre-line">
                      {metric.label}
                    </div>
                  </div>
                ))}
            ...
            {FAQ_DATA.map((faq, idx) => (
              ...
              >
                <h3 className="text-sm md:text-base font-bold text-gray-900 flex justify-between items-center gap-4">
                  {faq.q}
                  <ChevronDown
                    className={`w-4 h-4 text-gray-400 transition-transform duration-300 shrink-0 ${openFaqIndex === idx ? "rotate-180 text-[#1881c4]" : ""}`}
                  />
                </h3>
                {openFaqIndex === idx && (
